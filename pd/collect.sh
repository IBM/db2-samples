...
then
	...
	db2trc on -Madd sqlbfix $memberall
	sleep 5
...
db2 +c "call REORGCHK_TB_STATS('T','ALL')" > $HDIR/REORGCHK_TB_STATS.`hostname`.out
db2 +c "select table_schema, table_name,DATAPARTITIONNAME,card,overflow,  f1,  f2,  f3,  reorg  from SESSION.TB_STATS where REORG LIKE '%*%'" > $HDIR/NeedTSReorg.`hostname`.out
db2 +c "CALL SYSPROC.REORGCHK_IX_STATS('T', 'ALL')" > $HDIR/REORGCHK_IX_STATS.`hostname`.out
db2 +c "SELECT TABLE_SCHEMA,TABLE_NAME,INDEX_SCHEMA,INDEX_NAME,DATAPARTITIONNAME,INDCARD,F4,F5,F6,F7,F8,REORG FROM SESSION.IX_STATS  WHERE REORG LIKE '%*%'" > $HDIR/NeedTXReorg.`hostname`.out
db2 "select TABSCHEMA,TABNAME,CREATE_TIME,ALTER_TIME,INVALIDATE_TIME,STATS_TIME,COLCOUNT,TABLEID,TBSPACEID,CARD,NPAGES,MPAGES,FPAGES,OVERFLOW,LASTUSED,TABLEORG from syscat.tables WHERE TABSCHEMA NOT LIKE 'SYS%' AND TYPE = 'T' order by STATS_TIME,TABSCHEMA,TABNAME " > $HDIR/syscat_tables.`hostname`.out		
