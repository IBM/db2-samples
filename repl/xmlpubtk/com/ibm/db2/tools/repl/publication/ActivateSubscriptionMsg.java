...
/**
 * The ActivateSubscriptionMsg sends a message to QCapture asking it to invalidate the send
 ...
 * activate the subscription "SubscriptionName" at the QCapture program.
 *
 * @author tjacopi
 *
 */
public class ActivateSubscriptionMsg extends ControlMsg {
   ...
   */
  public ActivateSubscriptionMsg(String subscriptionName) {
  ...
  protected void validate() throws Exception {
    if (subscriptionName == null || subscriptionName.length() == 0 ) {                // Insure we have a queue name
      throw new Exception("A queue name must be specified for an ActivateSubscriptionMsg");
    };
  }
  ...
  protected void generateXML(StringBuffer xmlDocument) {
    xmlDocument.append("<activateSub subName=\"");
