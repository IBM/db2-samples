--********************************************************************/
--                                                                   */
--          IBM InfoSphere Replication Server                         */
--      Version 10 for Linux, UNIX AND Windows                      */
--                                                                   */
--     Sample Q Replication migration script for UNIX AND NT         */
--     Licensed Materials - Property of IBM                          */
--                                                                   */
--     (C) Copyright IBM Corp. 1993, 2011. All Rights Reserved       */
--                                                                   */
--     US Government Users Restricted Rights - Use, duplication      */
--     or disclosure restricted by GSA ADP Schedule Contract         */
--     with IBM Corp.                                                */
--                                                                   */
--********************************************************************/
-- Script to migrate Q Apply control tables from V97FP3 to V10.
-- Q Apply Migration script (asnqappluwv10.sql)
-- Prior to running this script, customize it to your existing 
-- Q Apply server environment:
-- (1) Locate and change all occurrences of the string !APPSCHEMA! 
--     to the name of the Q Capture schema applicable to your
--     environment
-- (2) Locate and change all occurrences of the string !APPTABLESPACE! 
--     to the name of the tablespace where the Q Apply control tables 
--     are created
-- (3) Run the script to migrate control tables into V10.
--

ALTER TABLE !APPSCHEMA!.IBMQREP_APPLYPARMS 
  ADD COLUMN TRACE_DDL CHAR(1) NOT NULL WITH DEFAULT 'N';
ALTER TABLE !APPSCHEMA!.IBMQREP_APPLYPARMS   
  ADD COLUMN REPORT_EXCEPTIONS CHARACTER(1) NOT NULL WITH DEFAULT 'Y';
ALTER TABLE !APPSCHEMA!.IBMQREP_APPLYPARMS   
  ADD COLUMN ORACLE_EMPTY_STR CHARACTER(1) NOT NULL WITH DEFAULT 'N';
ALTER TABLE !APPSCHEMA!.IBMQREP_APPLYPARMS   
  ADD COLUMN LOGMARKERTZ CHARACTER(8) NOT NULL WITH DEFAULT 'GMT';
  
UPDATE !APPSCHEMA!.IBMQREP_APPLYPARMS SET ARCH_LEVEL = '1001';  
  
  
CREATE TABLE  !APPSCHEMA!.IBMQREP_TAB_PROF
(SRC_SCHEMANAME VARCHAR(128) NOT NULL,
 SRC_TABNAME    VARCHAR(128) NOT NULL WITH DEFAULT '%',
 TRG_SCHEMANAME VARCHAR(128) NOT NULL WITH DEFAULT ,
 TSPROFNAME     VARCHAR(128))
   IN !APPTABLESPACE!;
 
 
ALTER TABLE !APPSCHEMA!.IBMQREP_TARGETS 
  ADD COLUMN SCHEMA_SUBNAME VARCHAR(64);
ALTER TABLE !APPSCHEMA!.IBMQREP_TARGETS   
  ADD COLUMN SUB_CREATOR VARCHAR(12) WITH DEFAULT NULL; 

ALTER TABLE !APPSCHEMA!.IBMQREP_RECVQUEUES
  ALTER COLUMN BROWSER_THREAD_ID SET DATA TYPE VARCHAR(9);

REORG TABLE !APPSCHEMA!.IBMQREP_RECVQUEUES;
